...
export default function LoginPage() {
  ...
  const reset = () => { setError(""); setSuccess(false); setSuccessEmail(""); };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Mail de bienvenue au premier login via localStorage
      const pendingKey = 'pendingWelcome_' + cred.user.email;
...
const handleRegister = async (e) => {
  ...
  try {
    ...
    try {
      ...
      console.error("Erreur sendEmailVerification:", e.message, e.code);
    }

    await auth.signOut();

    // Stocke le flag pour l'email de bienvenue au premier login
    localStorage.setItem('pendingWelcome_' + email, '1');
      ...
      <div className="auth-left">
        ...
        {success && (
          <div className="auth-success">
            ...
            <div className="auth-success-title">Vérifie ta boîte mail</div>
            <div className="auth-success-email">{successEmail}</div>
            <div className="auth-success-text">
              Un lien d'activation a été envoyé à cette adresse.<br/>
              Clique dessus pour activer ton compte et accéder à NextGen Gaming.
            </div>
            <div className="auth-success-note">Pense à vérifier tes spams si tu ne le reçois pas.</div>
          </div>
        )}
